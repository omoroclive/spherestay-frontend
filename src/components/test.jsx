  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <ToastContainer
        ...
        draggable
        pauseOnHover
        className="mt-16" // Add some margin to avoid overlapping with test banner
      />
...
const PublicPropertiesDetails = () => {
  ...
  // API call for property details if not available in state
  ...
  useEffect(() => {
    ...
    if (property && !property._viewIncremented) {
      ...
      // Note: Actual implementation would require an API endpoint
 ...
 const formatOperatingHours = (operatingHours) => {
  ...
  const currentDay = days[currentDayIndex === 0 ? 6 : currentDayIndex - 1]; // Adjust for Sunday (0) to map to 'sunday'
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="fixed bottom-6 right-6 flex flex-col gap-3 z-30">
        <motion.button
          onClick={() => setIsFavorite(!isFavorite)}
          className={`w-14 h-14 rounded-full shadow-lg backdrop-blur-sm transition-all duration-300 hover:scale-110 ${
            isFavorite 
        ...
        <motion.button
          onClick={() => setShowShareModal(true)}
          className="w-14 h-14 rounded-full bg-[#E83A17] text-white shadow-lg backdrop-blur-sm transition-all duration-300 hover:scale-110 hover:bg-[#c53214]"
          whileHover={{ scale: 1.1 }}
          whileTap={{ scale: 0.95 }}
          aria-label="Share"
        >
        ...
        <motion.button
          ...
          className="w-14 h-14 rounded-full bg-gray-800 text-white shadow-lg backdrop-blur-sm transition-all duration-300 hover:scale-110 hover:bg-gray-700"
